(() => {
  ...
  let n = null,
    ...
    d = null;
  const s = () => {
      d && (window.clearTimeout(d), d = null), n = null, i = null;
    },
    c = () => n;
  e.registerComponent({
    name: "tap-to-place",
    ...
    stateMachine: ({
      ...
      l("initial").initial().listen(s, e.input.SCREEN_TOUCH_START, l => {
        l.data.worldPosition && (d = window.setTimeout(() => {
          (d => {
            const l = !n;
            n = n || t.createEntity(c.get(s).prefab), i = t;
            const p = t.getEntity(n);
            p.setLocalPosition(d), p.set(e.Scale, {
              x: o * r,
              y: o * r,
              z: o * r
            }), l && (p.set(e.Quaternion, e.math.quat.yRadians(Math.random() * Math.PI)), e.PositionAnimation.set(t, n, {
              duration: 650,
              loop: !1,
              fromX: d.x,
              fromY: d.y - 1.2,
            ...
            })), t.events.dispatch(s, a);
          })(l.data.worldPosition), d = null;
        }, 90));
      }).listen(s, e.input.GESTURE_START, () => {
        d && (window.clearTimeout(d), d = null);
      });
    }
  });
  const l = e.defineQuery([t]),
    p = [{
      label: "Hood",
    ...
    stateMachine: ({
      ...
      const m = () => {
        ...
        const n = o.matrixWorldInverse.elements,
          i = o.projectionMatrix.elements;
        l(a).forEach(e => {
          const o = t.get(a, e),
            r = b.find(e => e.dataset.hotspot === o.label);
          if (r) try {
            const t = a.transform.getWorldPosition(e),
              ...
              l = i[0] * o + i[4] * d + i[8] * s + i[12] * c,
              p = i[1] * o + i[5] * d + i[9] * s + i[13] * c,
              b = i[3] * o + i[7] * d + i[11] * s + i[15] * c;
            if (b <= 0) return;
            const u = (l / b * .5 + .5) * window.innerWidth,
              f = (-p / b * .5 + .5) * window.innerHeight;
            r.style.display = "block", r.style.transform = `translate(-50%, -50%) translate(${u}px, ${f}px)`;
          } catch {
            return;
          }
        });
      };
      d("initial").initial().onEnter(() => {
        const t = document.createElement("div");
        t.id = "supra-ui-root", t.innerHTML = '\n  <style>\n    #supra-loading, #supra-ar-hud { font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif; }\n    #supra-loading { position: fixed; inset: 0; z-index: 10000; display: flex; align-items: center; box-sizing: border-box; padding: 24px; background: #050908; color: #f5fffc; transition: opacity .45s ease, visibility .45s ease; }\n    #supra-loading.is-hidden { opacity: 0; visibility: hidden; pointer-events: none; }\n    .supra-loading-content { width: min(100%, 380px); margin: auto; }\n    .supra-loading-title { margin: 0; font-size: 32px; line-height: 1.08; font-weight: 760; letter-spacing: 0; }\n    .supra-loading-copy { margin: 12px 0 26px; color: #afc6bd; font-size: 15px; line-height: 1.45; }\n    .supra-progress { height: 3px; width: 100%; overflow: hidden; border-radius: 3px; background: rgba(255,255,255,.15); }\n    .supra-progress::before { content: ""; display: block; height: 100%; width: 42%; border-radius: inherit; background: #1ed7ac; animation: supra-progress 1.1s ease-in-out infinite; }\n    @keyframes supra-progress { from { transform: translateX(-110%); } to { transform: translateX(250%); } }\n    .supra-access-card { padding: 22px; border: 1px solid rgba(112,233,201,.34); border-radius: 8px; background: #10201c; box-shadow: 0 18px 50px rgba(0,0,0,.38); }\n    .supra-access-card p { margin: 10px 0 22px; color: #b4c9c1; font-size: 15px; line-height: 1.45; }\n    .supra-start { width: 100%; min-height: 50px; border: 0; border-radius: 7px; background: #1ed7ac; color: #04100d; font-size: 16px; font-weight: 800; cursor: pointer; }\n    .supra-start:disabled { opacity: .55; cursor: wait; }\n    #supra-ar-hud { position: fixed; right: 16px; bottom: calc(16px + env(safe-area-inset-bottom)); left: 16px; z-index: 9999; pointer-events: none; }\n    .supra-panel { display: grid; grid-template-columns: 1fr auto; gap: 14px 18px; align-items: center; box-sizing: border-box; padding: 15px 16px; border: 1px solid rgba(255,255,255,.3); border-radius: 8px; background: rgba(7, 18, 16, .84); box-shadow: 0 12px 36px rgba(0,0,0,.26); backdrop-filter: blur(14px); color: white; pointer-events: auto; }\n    .supra-name { font-size: 15px; font-weight: 760; letter-spacing: .04em; }\n    .supra-status { margin-top: 4px; color: #70e9c9; font-size: 12px; font-weight: 650; }\n    .supra-reset { width: 42px; height: 42px; display: grid; place-items: center; border: 1px solid rgba(255,255,255,.34); border-radius: 50%; background: rgba(255,255,255,.08); color: #fff; font-size: 24px; line-height: 1; cursor: pointer; }\n    .supra-scale { grid-column: 1 / -1; display: grid; grid-template-columns: 44px 1fr 50px; gap: 12px; align-items: center; }\n    .supra-scale label, .supra-scale output { font-size: 13px; font-weight: 700; }\n    .supra-scale output { color: #70e9c9; text-align: right; }\n    #supra-scale-input { width: 100%; accent-color: #1ed7ac; }\n    #supra-car-hotspots { position: fixed; inset: 0; z-index: 9998; pointer-events: none; }\n    .supra-car-hotspot { position: fixed; display: none; min-width: 54px; height: 28px; padding: 0 12px; border: 1px solid #76f0d2; border-radius: 14px; background: rgba(5, 55, 44, .94); box-shadow: 0 5px 16px rgba(0, 0, 0, .25); color: #fff; font: 750 11px/1 -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif; pointer-events: auto; }\n  </style>\n  <section id="supra-loading" aria-live="polite">\n    <div class="supra-loading-content">\n      <div class="supra-access-card">\n        <h1 class="supra-loading-title">See it on the ground</h1>\n        <p id="supra-access-copy">Allow motion access so the Supra stays anchored in your space.</p>\n        <button id="supra-start-ar" class="supra-start" type="button">Start AR</button>\n      </div>\n    </div>\n  </section>\n  <aside id="supra-ar-hud">\n    <div class="supra-panel">\n      <div><div class="supra-name">TOYOTA GR SUPRA</div><div class="supra-status">AR Preview</div></div>\n      <button class="supra-reset" type="button" aria-label="Reset car" title="Reset car">&#8635;</button>\n      <div class="supra-scale">\n        <label for="supra-scale-input">Scale</label>\n        <input id="supra-scale-input" type="range" min="0.5" max="1.75" step="0.05" value="1" aria-label="Car scale">\n        <output for="supra-scale-input">1.00x</output>\n      </div>\n    </div>\n  </aside>\n  <div id="supra-car-hotspots"></div>\n', document.body.appendChild(t), u = document.querySelector("#supra-loading");
        const a = document.querySelector("#supra-ar-hud"),
          d = document.querySelector("#supra-start-ar"),
          c = document.querySelector("#supra-access-copy"),
          l = document.querySelector("#supra-scale-input"),
          m = document.querySelector(".supra-scale output"),
          h = document.querySelector(".supra-reset"),
          g = document.querySelector("#supra-car-hotspots"),
          y = e => e.stopPropagation();
        b = p.map(e => {
          const t = document.createElement("button");
          return t.className = "supra-car-hotspot", t.type = "button", t.dataset.hotspot = e.label, t.textContent = e.label, t.addEventListener("pointerdown", y), t.addEventListener("touchstart", y), t.addEventListener("click", () => window.location.assign(e.destination)), g.appendChild(t), t;
        }), a?.addEventListener("pointerdown", y), a?.addEventListener("touchstart", y), l.addEventListener("input", () => {
          const t = Number(l.value);
          m.value = `${t.toFixed(2)}x`, m.textContent = m.value, (t => {
            r = t, n && i && i.getEntity(n).set(e.Scale, {
              x: o * r,
              y: o * r,
              z: o * r
            });
          ...
          (() => {
            ...
            e.PositionAnimation.set(a, t, {
              ...
              n === t && (a.deleteEntity(t), s());
            }, 650);
          })();
        }), d.addEventListener("click", async () => {
          d.disabled = !0, c.textContent = "Requesting motion access...";
          try {
            const e = window.DeviceMotionEvent,
              t = window.DeviceOrientationEvent,
              a = [e.requestPermission?.(), t.requestPermission?.()].filter(e => Boolean(e));
            if ((await Promise.all(a)).some(e => "granted" !== e)) return d.disabled = !1, void (c.textContent = "Motion access is needed to anchor the car on the ground.");
            f = !0, u?.classList.add("is-hidden");
          } catch {
            d.disabled = !1, c.textContent = "Unable to request motion access. Please try again.";
          }
        });
        let x = null;
        const v = t => {
          if (2 !== t.touches.length || t.target instanceof Element && t.target.closest("#supra-ar-hud")) return void (x = null);
          const [a, o] = [t.touches[0], t.touches[1]],
            r = Math.atan2(o.clientY - a.clientY, o.clientX - a.clientX);
          var d;
          null !== x && (d = x - r, n && i && i.getEntity(n).rotateSelf(e.math.quat.yRadians(d))), x = r;
        };
